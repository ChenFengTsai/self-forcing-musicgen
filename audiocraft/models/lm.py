...
#     T_roll: int,
# ) -> tp.Dict[str, dict]:
#     """Slice per_step_ce into isolated temporal windows and return stats."""
#     window_stats: tp.Dict[str, dict] = {}
    
#     prev_sec = 0
#     prev_frame = 0

#     # Ensure windows are processed in chronological order
#     for w_sec in sorted(window_sizes_sec):
#         end_frame = int(w_sec * frame_rate)
#         end_frame = min(end_frame, T_roll)

#         if end_frame <= prev_frame:
#             continue

#         # Slice only the current isolated window
#         window_ce = per_step_ce[prev_frame:end_frame].mean().item()
        
#         # Name the key explicitly, e.g., "10-20s"
#         key = f"{prev_sec}-{w_sec}s"
        
#         window_stats[key] = {
#             "mean_ce":    round(window_ce, 6),
#             "start_frame": prefix_len + prev_frame,
#             "end_frame":   prefix_len + end_frame,
#             "n_frames":    end_frame - prev_frame,
#         }
        
#         # Advance the boundaries
...
#     #     alpha: float = 2.0,
#     #     beta: float = 0.25,   # Unused in frame-level, kept so your config doesn't crash
#     #     gamma: float = 2.0,   # Unused in frame-level, kept so your config doesn't crash
#     #     c_min: float = 0.35,
#     #     c_max: float = 0.85,
...
#     #             # Extract confidence across the vocab dimension
#     #             confidence = first_logits.to(torch.float32).softmax(dim=-1).max(dim=-1).values  # [B, K, T]
                
#     #             # NEW: Frame-level confidence [B, 1, T] - average across codebooks
#     #             frame_confidence = confidence.mean(dim=1, keepdim=True)
...
#     #     # ---- Compute SASS replacement probability p_bt [B, 1, T] ----

#     #     # 2. Temporal gate g_time(t) — [T]
#     #     t_idx = torch.arange(T, device=codes.device, dtype=torch.float32)
#     #     tau = t_idx / (T - 1 + eps)
#     #     g_time = tau.pow(alpha).unsqueeze(0).unsqueeze(0)  # [1, 1, T]

#     #     # 3. Confidence gate g_conf(b,1,t) — [B, 1, T]
#     #     g_conf = ((frame_confidence - c_min) / (c_max - c_min + eps)).clamp(0.0, 1.0)

#     #     # Combine into frame-level probabilities
#     #     p_bt = (
#     #         p_max
#     #         * g_warmup
#     #         * g_time
...
#     #     p_bt = p_bt.nan_to_num(nan=0.0, posinf=token_cap, neginf=0.0).clamp(min=0.0, max=token_cap)

#     #     # ---- Sample Bernoulli mask at FRAME level ----
#     #     mask_replace_frame = torch.bernoulli(p_bt).bool()  # [B, 1, T]
        
#     #     # Expand to all codebooks so the entire frame is replaced together
#     #     mask_replace = mask_replace_frame.expand(-1, K, -1) # [B, K, T]
...
#     #         # Calculate average probability over valid frame timesteps
#     #         valid_frame_mask = valid_mask[:, 0:1, :]
#     #         avg_p = p_bt[valid_frame_mask].mean().item() if valid_frame_mask.any() else 0.0
...
#     #             f"MODE: FRAME-LEVEL (Synchronized)"
#     #         )
#     #     # ==========================================
...
#         # ---- SASS hyperparameters ----
#         current_step: int = 0,
#         p_max: float = 0.15,      # Lowered from 0.5
#         warmup_start: int = 5000,
#         warmup_ramp: int = 8000,
#         alpha: float = 2.0,       # Parabolic time curve
#         beta: float = 0.0,
#         gamma: float = 0.0,
#         c_min: float = 0.35,
#         c_max: float = 0.85,
#         token_cap: float = 0.15,  # Lowered from 0.5
#     ) -> LMOutput:
...
#                     stage=stage, keep_only_valid_steps=keep_only_valid_steps,
#                 )
#                 first_logits = first_pass.logits  # [B, K, T, vocab]
                
#                 # Extract confidence across the vocab dimension (using max prob for the gate)
#                 confidence = first_logits.to(torch.float32).softmax(dim=-1).max(dim=-1).values  # [B, K, T]
...
#                 # ==========================================
#                 # --- NEW: TOP-K TEMPERATURE SAMPLING ---
#                 # ==========================================
#                 temp = 1.0
#                 top_k = 250
                
#                 # 1. Immediately cast to FP32 to prevent underflow/overflow
#                 scaled_logits = first_logits.to(torch.float32) / temp
                
#                 # 2. Sanitize any NaNs or extreme values from padded regions
#                 scaled_logits = scaled_logits.nan_to_num(nan=float('-inf'))
...
#                 # 5. SAFETY NET: If a padded row became all NaNs after softmax, 
#                 # replace it with a uniform distribution so multinomial doesn't crash.
#                 # (These dummy tokens will be thrown out by your mask_replace later anyway).
#                 probs = probs.nan_to_num(nan=0.0)
...
#                 pred_tokens = torch.multinomial(probs_flat, 1).view(B, K, T)
#                 # ==========================================
                
#                 # Explicitly delete references to free memory before Pass 2
#                 del first_pass, first_logits, scaled_logits, probs, probs_flat
#         finally:
#             if was_training:
#                 self.train()
...
class LMModel(StreamingModule):
    ...
    def compute_sass_predictions(
        ...
        warmup_start: int = 5000,
        warmup_ramp: int = 8000,
        decay_ratio: float = 0.7,       # decay p_max to (1-decay_ratio)*p_max after peak
        alpha: float = 1.0,             # linear temporal curve
        c_min: float = 0.35,
        c_max: float = 0.85,
        token_cap: float = 0.20,
        codebook_weights: tp.Optional[tp.List[float]] = None,  # weight each codebook's confidence contribution
        temp: float = 1.0,        # <-- ADD THIS
        top_k: int = 250,         # <-- ADD THIS
    ) -> LMOutput:

        B, K, T = codes.shape
        eps = 1e-8

        # 1. Warmup-then-decay schedule for g_schedule
        #    Ramps up from 0→1 during [warmup_start, warmup_start+warmup_ramp],
        #    then decays from 1→(1-decay_ratio) over remaining training.
        if current_step < warmup_start:
            g_schedule = 0.0
        else:
            ramp_end = warmup_start + warmup_ramp
            if current_step < ramp_end:
                # Ramp phase: 0 → 1
                g_schedule = (current_step - warmup_start) / (warmup_ramp + eps)
            else:
                # Decay phase: 1 → (1 - decay_ratio)
                decay_steps = max(total_steps - ramp_end, 1)
                decay_progress = (current_step - ramp_end) / decay_steps
                g_schedule = 1.0 - decay_ratio * decay_progress
            g_schedule = max(0.0, min(1.0, g_schedule))
        ...
        # ---- Pass 1: teacher-forced (detached, KEEP training mode for consistent dropout) ----
        with torch.no_grad():
            first_pass = self.compute_predictions(
                codes, conditions, condition_tensors,
                stage=stage, keep_only_valid_steps=keep_only_valid_steps,
            ...
            # Per-codebook confidence [B, K, T]
            confidence = first_logits.to(torch.float32).softmax(dim=-1).max(dim=-1).values

            # ---- Codebook-weighted frame confidence [B, 1, T] ----
            # Instead of simple mean, weight each codebook's confidence contribution.
            # Higher weight on harder codebooks (q3, q4) means: we prefer to replace
            # frames where the hard codebooks are confident (= the model can produce
            # coherent replacements across all codebooks for that frame).
            # q1 being confident is necessary but not sufficient — if q3/q4 are
            # uncertain, the predicted frame would be incoherent.
            if codebook_weights is not None:
                cw = torch.tensor(codebook_weights[:K], device=codes.device, dtype=torch.float32)
                cw = cw / (cw.sum() + eps)  # normalize to sum=1
                cw = cw.view(1, K, 1)  # [1, K, 1]
                frame_confidence = (confidence * cw).sum(dim=1, keepdim=True)  # [B, 1, T]
            else:
                frame_confidence = confidence.mean(dim=1, keepdim=True)  # [B, 1, T]

            # ---- Top-K sampling for replacement tokens ----
            scaled_logits = first_logits.to(torch.float32) / temp ### this line is new
            scaled_logits = scaled_logits.nan_to_num(nan=float('-inf'))
        ...
        # ---- Compute SASS replacement probability ----

        # 2. Temporal gate g_time(t) — linear (alpha=1) or sublinear
        t_idx = torch.arange(T, device=codes.device, dtype=torch.float32)
        tau = t_idx / (T - 1 + eps)
        g_time = tau.pow(alpha).unsqueeze(0).unsqueeze(0)  # [1, 1, T]

        # 3. Confidence gate g_conf — only replace where model is confident
        #    With codebook_weights, this already emphasizes frames where the
        #    harder codebooks are confident (= safe to replace the whole frame)
        g_conf = ((frame_confidence - c_min) / (c_max - c_min + eps)).clamp(0.0, 1.0)

        # 4. Combine into frame-level probability [B, 1, T]
        p_bt = p_max * g_schedule * g_time * g_conf
        p_bt = p_bt.nan_to_num(nan=0.0, posinf=token_cap, neginf=0.0).clamp(min=0.0, max=token_cap)

        # 5. Frame-level Bernoulli sampling, then expand to all codebooks
        #    Entire frames are replaced together to maintain inter-codebook coherence
        mask_replace_frame = torch.bernoulli(p_bt).bool()  # [B, 1, T]
        mask_replace = mask_replace_frame.expand(-1, K, -1)  # [B, K, T]

        # Do not replace special padding tokens
        valid_mask = (codes != self.special_token_id)
        mask_replace = mask_replace & valid_mask
